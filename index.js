// MDFF_options.duration_time
var MDFF_options={
	duration_time:0,
	use_ascore_as:true,
	total_score:114514
}
MDFF_options.I_want_it=()=>{

}
$.getScript("https://cdn.bootcdn.net/ajax/libs/dat-gui/0.7.7/dat.gui.min.js",()=>{
	var gui=new dat.GUI()
	gui.add(MDFF_options,"duration_time").name("自定义用时")
	/*gui.add(MDFF_options,"use_ascore_as").name("使用总分作为得分")
	gui.add(MDFF_options,"total_score").name("自定义得分")*/
})
/**
 * 录音模块加载等待对话框
...
//			console.log('停止录音');
	...
	var practice = TSP.practice = {
		...
		isAutoPractice : function(){
			...
			if(mode == 'exam' 
				&& (	// 单元测试、人机对话、作业的考试模式
						source == 'ts' || source == 'unit' || source == 'hw'
					)
			){
				TSP.practice.start();
			}
			...
			$('.sub_test_area').each(function(x, y){
				...
				$(y).find('.test_content').each(function(i, n){
					...
					$(n).find('.question_content').each(function(j, m){
		    			...
		    			if(test_mold == 1 && !($.inArray(parseInt(sub_type), [1320, 1321, 1323, 1324, 1326, 1621, 1626, 1631]) > -1)){
		    				// 正确答案，A...
...
//		    					$(m).find('input[type=radio]:eq(' + right_answer + ')').attr('checked', 'checked').closest('label').addClass('radio_right_answer');
		    			...
		    			// 填空形式、北师大题型1为填空题
		    			else if((test_mold == 2 && main_type != 2700) 
		    					|| (test_mold == 1 && $.inArray(parseInt(sub_type), [1320, 1321, 1323, 1324, 1326, 1621, 1626, 1631]) > -1)){
		    				// 正则表达式
		    			...
		    			// 下拉列表形式
	    					...
	    					if(user_answer_array[j] == right_answer){
		        				...
		        				if(!hw_not_submit){
		        					$(m).find('select').after('<span class="right_answer" style="font-size: 14px">('+$(m).find('select option[value="'+right_answer+'"]').text()+')</span>');
		        				}
			    				// 答题卡颜色变化
		        				$('.p_answer_list ul li[data-index='+qid+']').addClass('answer_list_wrong_class');
		    				}
		    			...
		    			// 录音形式
	    					...
	    					// 问题添加按钮 
	    					if($(n).attr('data-subtype') == 1540 || $(n).attr('data-subtype') == 1541 || $(n).attr('data-subtype') == 1544){
		...
		start : function(){
			...
			self_practice_key = setInterval(function(){
				...
				// 是否存在笔试
            	var hasBs = $('.test_content[data-kind="3"]').length ? true : false;
				...
				if(obj == undefined || obj.length == 0){
					if(hasBs){
	                    ...
	                    var self_answer_key = setInterval(function(){
	                    	...
	                    	// 判分结束或等待时间结束
	                    	if($('.p_tests_area').attr('data-page') != 'result' && res){
                    			// 停止时间
        						clearInterval(self_answer_key);
...
//					$('.test_content[data-id="'+tid+'"] .question_container .speak_sentence').attr("data-time_flag",time_flag);
	    						...
	    						if(judge_speaking){
			    					...
			    					if(video_question_num > 1 || sub_type == 1435 || sub_type == 1627){
			    							...
			    							if(quesiton_index == 1){
						    					...
						    					if(videoResult != undefined && videoResult != null 
						    						...
						    						$.each(videoResult[tid], function(i, obj){
						    							...
						    							if(result_index > 0){
						    								if(obj['result'] != undefined && obj['result']['score'] != undefined && result_index > 0){
							    								str += 'Q'+result_index+':'+obj['result']['score']+'&nbsp;';
							    							}else{
							    								str += 'Q'+result_index+':0&nbsp;';
							    							}
						    							}
			    						...
			    						if($('.test_content[data-id="'+tid+'"]').attr('data-type') == 1400 && sub_type != 1428 && sub_type != 1438
			    								&& !($('.p_paper_cnt').attr('data-source') == 'hw' && ($('.p_paper_cnt').attr('data-struct-type') == 2 || $('.p_paper_cnt').attr('data-struct-type') == 4))){
		    							...
		    							// 小学知识点试题分数显示在句子后面
			    							...
			    							speak_sen.removeClass('pass_font no_pass_font').html( sen_txt + '<span class="lccj_speak_sen_score">(' +sen_score + ')</span>');
			    						...
			    						// 小学连词成句
			    							...
			    							if(typeof practice_id && $.inArray(parseInt(practice_id), shids) > -1){
			    								...
			    								$('.test_content[data-id="'+tid+'"]').find('.left_area').html('得分：'+sen_score);
			    							}
			    						}
			    					}
	    						}
		...
		process : {
			...
			downloadAudioFile : function(){
				...
				// 添加音频文件
				$('.test_content,.qhlx_test_container').find('.p_Laudio').each(function(i, n){
					// 音频名称
			...
			calculateVideoTotalScoreForNew : function(tid, main_type, sub_type, scores, video_num, sen_num, isSenNum){
				...
				var examMode = false;
				if(
						$('.p_paper_cnt').attr('data-struct-type') == 2 || $('.p_paper_cnt').attr('data-struct-type') == 4
//						$('.p_paper_cnt').attr('data-source') == 'hw' 
//							&& $('.p_operation_box #test-mode').attr('data-mode') == 'exam'
				){
					examMode = true;
				}
			...
			calculateVideoTotalScore : function(tid, ascore){
				// 主类型
				var main_type = parseInt($('.test_content[data-id="' + tid + '"]').attr('data-type'));
				// 子类型
				var sub_type = $('.test_content[data-id="' + tid + '"]').attr('data-subtype');
			...
			submitHomework : function(answers, precord_id){
				...
				// 显示绑定微信按钮
				if(isBindWeixin != undefined && isBindWeixin == 0){
					$('.bindWeixin_btn').removeClass('hide');
				}
			},
			/**
			 * 提交答案检查口语情况
		    				...
		    				$('.test_content[data-kind="2"]').find('.question_li:not(.question_li_1520)').each(function(i, obj){
		    					if($(obj).html()){
		    						if($(obj).find('.btn_question_area').length){
			    						...
			    						if($(obj).closest('.test_content').attr('data-subtype') == 1540 
			    								|| $(obj).closest('.test_content').attr('data-subtype') == 1541
			    								|| $(obj).closest('.test_content').attr('data-subtype') == 1544){
    			...
    			if(!practice_data_save && $.isEmptyObject(practice_data)){
			    	$('.sub_test_area').each(function(r, s){
			    		...
			    		$(this).find('.test_content').each(function(i, n){
				    		...
				    		var ascore = 0;
				    		// 试题问题数
				    		var qsnum = $(this).attr('data-count');
				    		
				    		// 各试题
				    		answers[id] = {'id' : id, 'qstype' : main_type, 'score' : 0, 'answer' : '', 'ascore' : 0, 
				    				'qsnum' : qsnum, 'item_order' : 0, 'detail' : new Array(), 
				    		...
				    		$(this).find('.question_container').each(function(j, m){
				    			...
				    			// 单选框(复选框)形式、子类型不为北师大题型(北师大题型1为填空题)
					    		if(test_mold == 1 && !($.inArray(parseInt(sub_type), [1320, 1321, 1323, 1324, 1326, 1621, 1626, 1631]) > -1) && $(m).find('input[type=radio]').length > 0){
				    				// 不是第一题
				        			if(j > 0){
				        				// 答案
				        			...
				        			if(user_answer == right_answer){
			    						...
			    						if(judge_speaking){
					        				// 用户答案变色
					        				$(this).find('.question_content input:checked').closest('label').addClass('radio_wrong_answer');
					        				// 正确答案
											if(type != 'homework'){
												$(this).find('.question_content input[value="'+right_answer+'"]').closest('label').addClass('radio_right_answer');
											}
					        				// 答题卡颜色变化
					        				$('.p_answer_list ul li[data-index='+qid+']').addClass('answer_list_wrong_class');
			    						}
				    			...
				    			// 填空形式、北师大题型1为填空题
					    		}else if(((test_mold == 2 && main_type != 2700) || (test_mold == 1 && $.inArray(parseInt(sub_type), [1320, 1321, 1323, 1324, 1326, 1621, 1626, 1631]) > -1 )) && $(m).find('input[type=text]').length > 0){
				    				// 用户答案数组
				        			...
				        			// 如果当前试题为海口试题
				        			if($.inArray(parseInt(id), tids) > -1){
				        				// 计算总分
										ascore = qsnum * small_test_score;
										
				        				// 正确答案数据
				        				var right_answers = new Array();
				        				// 正确答案
				        				right_answers.push(right_answer.split('*'));
				        				// 是否有扩张答案
				        				if(ext_answer != undefined){
				        					var ext_answers = ext_answer.split('#');
				        					// 循环处理数据
				        					for(var ea in ext_answers){
				        						if(ext_answers[ea] != ''){
				        							// 正确答案
							        				right_answers.push(ext_answers[ea].split('*'));
				        						}
				        					}
				        				}
			        					...
			        					var isRight = 1;
			        					// 答对的情况
			        					var right_info = new Array();
			        					
			        					// 循环判断用户答案
			        					for(var ras = 0; ras < right_answers.length; ras++){
			        						// 答对数量
				        					rightNum = 0;
				        					// 是否答对
				        					isRight = true;
			        						...
			        						// 循环答案判断
			        						for(var ra = 0; ra < right_answers[ras].length; ra++){
			        							if(right_answers[ras][ra] == user_cmp_answer[ra]){
			        								rightNum++;
			        							}else{
			        								isRight = 0;
			        							}
			        						...
			        						right_info.push({right_num : rightNum, is_right : isRight});
			        					}
			        					...
			        					// 判断得分及对错
			        					for(var ri = 0; ri < right_info.length; ri++){
			        						if(right_info[ri]['right_num'] > rightNum){
			        							// 答对数量
					        					rightNum = right_info[ri]['right_num'];
					        					// 答对序号
					        					rightIndex = ri;
			        						}
			        					}
			        					...
			        					// 得分
			        					score = rightNum * small_test_score;
			        					// 答对数量和空格数相同
					        			if(rightNum == qsnum){
					        				// 是否显示口语判分
				    						if(judge_speaking){
			    						...
			    						if(judge_speaking){
					        				...
					        				if(type != 'homework'){
					        					...
					        					$(this).find('.question_content input[type="text"]').each(function(ix, obj){
					        						// 循环判断用户答案
						        					for(var ras = 0; ras < right_answers.length; ras++){
						        						$(this).after('<span class="right_answer">('+right_answers[ras][ix]+')</span>');
						        					}
						        				});
					        				}
			    						}
	...
	//			        				$(this).find('.question_content select').closest('label').addClass('radio_right_answer');
			    						...
			    						if(judge_speaking){
					        				// 用户答案变色
		//			        				$(this).find('.question_content select').closest('label').addClass('radio_wrong_answer');
					        				$(this).find('.question_content select').addClass('select_wrong');
					        				// 正确答案
						        				if(type != 'homework'){
						        					$(this).closest('.question_content').find('span.right_answer').remove();
						        					$(this).find('.question_content select').after('<span class="right_answer">('+right_answer+')</span>');
						        				}
					        				// 答题卡颜色变化
					        				$('.p_answer_list ul li[data-index='+qid+']').addClass('answer_list_wrong_class');
			    						}
	    		...
	    		if($('#test-mode[data-mode="free"]').is(':checked')){
		    		duration_time = parseInt(time);
		    	}else{
		    		// 自主出题的自由模式采用正计时
		    		duration_time = parseInt(total_time) - parseInt(count_down_time);
		    	}
		    	// 后台数据存储
	    		if(type == 'competition'){
	    			// 设置提交按钮不可用
				...
				// 页面统计信息
				if(MDFF_options.use_ascore_as=false){
					practice_score=MDFF_options.total_score
				}
		    	var record = {
					'time' : Math.ceil((duration_time)/60), 
					'practice_score' : Math.formatFloat(practice_score), 
					'practice_total_score' : practice_total_score.toString().length > 5 ? Math.formatFloat(practice_total_score) : practice_total_score};
		    	...
		    	if(!practice_data_save && !practice_data.practice_record){
			    	...
			    	var practice_record = {'version_id' : version_id, 'grade_id' : grade_id, 'unit' : unit_ids, 
			    			'practice_type' : type, 'practice_id' : practice_id, 'duration_time' : MDFF_options.duration_time, 
		    	...
		    	$.ajax({
					...
					success : function(data){
			    		...
			    		if(data.status && data.info.flag === true){
			    			if(type == 'competition'){
			    				// 页面统计信息
						    	var params = {prid : data.info.practice_record_id, 'duration_time' : MDFF_options.duration_time, 
			...
			showResult : function(result){
				$('.dialog_cnt').html($('#wrongresultInfoTemp').template(result));
				$('.dialog_cnt').dialog({
					dialogClass : 'small-dialog green-dialog',
					width : 480,
					modal : true,
			...
			backToSource : function(){
                ...
                source['/Practice/homework'] = new Array();
                source['/Practice/homework'][null] = '/Homework/lists';                  // 在线作业
                source['/Practice/homework']['wk'] = '/MicroLecture/weikeHomework';                       // 微课作业

                source['/Practice/tsSinglePractice'] = new Array();
                source['/Practice/tsSinglePractice']['ts'] = '/Paper/tsSingle';         // 人机对话题型突破
                source['/Practice/tsSinglePractice']['gy'] = '/Paper/TsSingleTestList';  // 纲要冲刺
				...
				source['/Practice/knowledgePractice']['weike'] = '/Teachingpurpose/index';           // 单元同步教学目标
				...
				source['/Practice/primaryPractice'] = '/Practice/smartPracticeList';		//小学的智能练习页面
                source['/Practice/microLecturePractice'] = new Array();
				source['/Practice/microLecturePractice']['microLecture'] = '/MicroLecture/index';       // 微课中心
				//获取当前文件路径
				...
				if(Array.isArray(source[path])){
					...
					if(path == '/Practice/knowledgePractice' && url_type == 'details'){
                        ...
                        var main_type =　TSP.practice.process.getQueryString("main_type");
                        var source_path = source[path][url_type] +'.html?main_type='+main_type
					}
                    else if(url_type == 'wk'){
                        var hid =　TSP.practice.process.getQueryString("hid");
                        var source_path = source[path][url_type] +'.html?hid='+hid
                    }
					else{
		...
		paperTest : {
			...
			setQjTest : function(){
				...
				$('.test_content[data-type="1500"]').each(function(i, obj){
					...
					if(sub_type == 1522 || sub_type == 1523 || sub_type == 1524 || sub_type == 1528 || sub_type == 1530 || sub_type == 1537 
							|| sub_type == 1539 || sub_type == 1540 || sub_type == 1544 || sub_type == 1545){
						// 问题
						var question_str = '';
						// 循环获取问题
						...
						// 显示问题
						if(sub_type == 1540 || sub_type == 1544 || sub_type == 1545){
			...
			select : function(param){
				...
				if(typeof param == 'number' && (page_mode || type == 'preview')){
					...
					// 定位试题
                    var li_top = 0;
					if($('.p_answer_list ul li[data-index=' + index + ']').length > 0){
                        li_top = $('.p_answer_list ul li[data-index=' + index + ']').offset().top;
					}
					// 答题卡顶部高度
                    var answer_list_top = 0;
                    if($('.p_answer_list').length > 0){
                        answer_list_top = $('.p_answer_list').offset().top;
					}
					// 滚动条位置
			 ...
			 * 设置选项长度
			 */
			setImageOption : function(obj){
				var obj = $(obj).closest('.question_content');
				//设置选项长度
				var sub_type = $(obj).closest('.test_content').attr('data-subtype');
				var lengthArr = new Array();
				var option_label = $(obj).find('.option_label');
				// 选项区域总宽度
				var width = $(obj).width();
				// 选项宽度数组
				option_label.each(function(i, n){
			...
			setImgSize : function(){
				$('.p_tests_area,.homework_box').find('img').each(function(i, n){
					...
					practice.paperTest.setImageOption(n);
				});
			},
			/**
			 * 试题加入题库或移除题库状态
			...
			initButton : function(){
				...
				if(!is_primary){
					...
					//添加左右箭头模板
//					$('.test_content[data-kind="1"] .p_operationBtn_container').find('.primary_btn_play').after($('#nextBtnCommom').template());
				}
				// 口语按钮
		...
		testTime : {
			...
			calculateTime : function(val){
				...
				if(page_mode){
					...
					var mode = $('.p_operation_box #test-mode').attr('data-mode');
					// 总时间(秒)
					if(mode == 'exam' && (source == 'ts' || source == 'unit'|| source == 'hw')){
			...
			countdown : function(){
				if($('#test-mode[data-mode="free"]').is(':checked')){
					$('.p_answerSheet_cnt .practice_time').html('用时');
					if(typeof time_id == 'undefined' || time_id == null){
			...
			timer : function(){
				...
				if(count_down_time < 0){
					...
					// 结构类型
					var struct_type = $('.p_paper_cnt').attr('data-struct-type');
					// 自动练习模式-- 单元测试、人机对话的考试模式、作业的试卷考试模式
					if(mode == 'exam' && (source == 'ts' || source == 'unit' || source == 'hw')){
						TSP.practice.process.submitAnswerCheck();
					}else if(type == 'competition' || type == 'competition_preview'){
						TSP.practice.process.submitAnswerCheck();
					}else{
						MessageBox({
...
//								testCtrls[main_type][main_type]['infos'][0].splice(1,2);
			...
			execute_ctrl : function(that){
				...
				if(act_type == 0){
					...
					// 等待时间
					var pracitce_total_time = parseInt($(obj).attr('data-wait-time'));
					...
					remainder_key = setInterval(function(){
						...
						// 进度条
						$('.test_ctrl_info_area .play_mp3_area .percentage_bule').css('width', ((pracitce_total_time - remainder_time)/pracitce_total_time * 100)+'%');
				...
				// 播放音频(题目说明、试题内容等)
					...
					// 试题音频
						...
						if(file_name != undefined && file_name != ''){
							...
							if(sub_type == 1442){
								// 音频文件
								file_name = $(obj).closest('.test_content').find('.speak_sentence').attr('data-mp3');
								// 播放音频
								practice.ctrlOpr.play_video(that, file_name);
							// 图片排序没有步骤问题音频，进入播放图片排序音频步骤
							}else if(sub_type == 1121 || sub_type == 1122 || sub_type == 1123 || sub_type == 1124){
				...
				// 看图说话
					...
					if(mp3_type == 10){
						...
						if(firstsen != undefined && firstsen.length > 0){
							...
							// 音频总时间
							var pracitce_total_time = end_time/1000.0 - start_time/1000.0;
							...
							play_key = setInterval(function(){
								...
								// 进度条
								$('.test_ctrl_info_area .play_mp3_area .percentage_bule').css('width', (remainder_time/pracitce_total_time * 100)+'%');
								
								if(pracitce_total_time - remainder_time <= 0){
									// 停止时间
									clearInterval(tape_remainder_key);
									// 停止录音
									TSP.audio.player.stop();
			...
			play_video : function(that, file_name){
				...
				// 音频总时间
				var pracitce_total_time = TSP.audio.player.audioElem.duration;
				// 循环倒计时
				play_key = setInterval(function(){
					// 音频播放当前时间
			...
			play_video_tppx : function(that){
				...
				// 音频总时间
				var pracitce_total_time = (end_time - start_time).toFixed(3);
				// 播放音频
				TSP.audio.player.play();
				// 提示语
				...
				play_key = setInterval(function(){
					...
					$('.test_ctrl_info_area .play_mp3_area .percentage_bule').css('width', (procee_time/pracitce_total_time * 100)+'%');
			...
			ky_read_video : function(obj, that){
				...
				// 音频总时间
				var pracitce_total_time = (end_time - start_time)/1000.0;
				...
				$('.test_ctrl_info_area .waveform_container').show();
				// 等待时间
				var remainder_time = practice.util.getRecordFunction(pracitce_total_time) * 1000;
				// 显示秒
				$('.test_ctrl_info_area .play_mp3_area .remainder_time_area').hide();
				// 步骤说明
				$('.test_ctrl_info_area .info_hint').html('初始化录音');
			...
			play_ky_gd_video : function(obj, that){
				...
				play_key = setInterval(function(){
					...
					if(pracitce_total_time - remainder_time <= 0){
						...
						if(!flag){
							// 下一步骤(开始录音)
							practice.ctrlOpr.tape_video(obj, that, pracitce_total_time, v_obj);
							// 标识
							flag = true;
						}
					}
			 ...
			 * 跟读录音
			 */
			tape_video : function(obj, that, pracitce_total_time, v_obj){
				// 隐藏进度条
				$('.test_ctrl_info_area .percentage_gray').hide();
				// 隐藏录音进度条
				$('.test_ctrl_info_area .waveform_container').show();
			...
			play_ky_li_video : function(obj, that, index){
				...
				var sub_type = $(obj).closest('.test_content').attr('data-subtype');
				// 情景对话
				if(main_type == 1500 && sub_type != 1501 && sub_type != 1503 && sub_type != 1509 && sub_type != 1510 && sub_type != 1512 
						&& sub_type != 1514 && sub_type != 1516 && sub_type != 1520 && sub_type != 1522 && sub_type != 1523 && sub_type != 1524 
						&& sub_type != 1526 && sub_type != 1527 && sub_type != 1528 && sub_type != 1529 && sub_type != 1530 && sub_type != 1537 
			...
			play_audio_tppx : function(that){
				...
				$(that).closest('.test_content').find('.images_sort_cnt .images_sort_option_cnt').each(function(i, n){
					...
					pracitce_total_time += (audio_arr[i].end_time - audio_arr[i].start_time);
				});
				...
				var play_img_sen = function(i){
					...
					play_key = setInterval(function(){
						...
						// 进度条
						$('.test_ctrl_info_area .play_mp3_area .percentage_bule').css('width', ((time_played + process_time) / pracitce_total_time * 100)+'%');
						
						$('.test_ctrl_info_area .remainder_time_area').show();
						// 剩余播放时间
						$('.test_ctrl_info_area .play_mp3_area .remainder_time').text(parseInt(pracitce_total_time - time_played - process_time));
...
$(function(){
    //隐藏纲要悬浮框
	if(typeof gangYaoInited != "undefined"){
        if(gangYaoInited){
            $('#jumpToGangYao').hide();
            clearInterval(gangYaoTimer);
            gangYaoInited = false;
        }
	}
			...
			'.btn_play:not(.disabled)':function(){
				...
				if(typeid == 1000 || typeid == null || typeid == undefined){
					...
					var part_ids = ['2703', '3282', '3723', '3730', '3731', '3732', '3931', '3932', '3933', '3934', '3994', '4025', '4034', 
						...
						'5921', '6881', '6928', '6929', '6934', '6935', '6940', '6941', '6998', '7028', '7279', '7280', '7281', '7282', '7283', 
						'7289', '7290', '7291', '7292', '7293'
					];
					// 如果为自由练习模式
					if($('#test-mode[data-mode="free"]').is(':checked') || ($('.p_paper_cnt').attr('data-source') == 'hw' && $('.p_test_part').attr('data-paper-type') == 1)){
						// 提示音、标题、等待时间
						...
						if(first_id == tid 
								&& (
										...
										|| (type == 'homework' && $('#test-mode[data-mode="exam"]').is(':checked'))
									)
							...
							var wait_part_ids = ['2702', '2703', '3338', '3723', '3930', '3931', '3932', '3933', '3934', '3936', '3988', '3995', '4023',
								...
								'7279', '7280', '7281', '7282', '7283', '7289', '7290', '7291', '7292', '7293'];
							// 准备时间
					...
					$('.test_ctrl_info_area').show();
					if(is_primary){
//						if($('.play_process_cnt.percentage_gray').hasClass('hide')){
							$('.play_process_cnt.percentage_gray').removeClass('hide').show();
							$('.remainder_time_area.dib').removeClass('hide').show();
//						}
					}
					// 隐藏录音进度条
					$('.test_ctrl_info_area .waveform_container').hide();
					...
					}else if(main_type == 1500 && sub_type != 1501 && sub_type != 1520 && sub_type != 1510 && sub_type != 1512 
							&& sub_type != 1514 && sub_type != 1516 && sub_type != 1522 && sub_type != 1523 && sub_type != 1524 
							&& sub_type != 1526 && sub_type != 1527 && sub_type != 1528 && sub_type != 1529 && sub_type != 1530 
							&& sub_type != 1537 && sub_type != 1539 && sub_type != 1540){
						// 保留数据
			...
			'.btn_answer' : function(){
				...
				if ($(this).is('.clicked')){
                    ...
                    var unit = '';
                    // 教材
                    var versionId = $('.p_paper_cnt').attr('data-version-id');
                    if(typeof versionId != 'undefined'){
                        version = versionId;
                    }else{
                        var versionId = $('.p_paper_cnt').attr('data-version');
                        if (typeof versionId != 'undefined') {
                            version = versionId;
                        }
                    }
                    // 年级
                    var gradeId = $('.p_paper_cnt').attr('data-grade-id');
                    if(typeof gradeId != 'undefined'){
                        grade = gradeId;
                    }else{
                        var gradeId = $('.p_paper_cnt').attr('data-grade');
                        if (typeof gradeId != 'undefined') {
                            grade = gradeId;
                        }
                    }
                    // 单元
                    var unitId = $('.p_paper_cnt').attr('data-unit-id');
                    if(typeof unitId != 'undefined'){
                        unit = unitId;
                    }

                    var ids = '';
                    $(this).closest('.test_content').find('.p_knowledge_points .knowledge_point').each(function(w, obj){
                        // 收集知识点ids
                    ...
                    // 判断是否存在知识点
					if (ids != ''){
                        var param = {'version' : version, 'grade' : grade, 'unit' : unit, 'ids' : ids};
                        param.version_alias_id = 1;
                        LoadingMessageBox('查询知识点微课中...');
                        $.post(TinSoConfig.student +'/Teachingpurpose/getWeikeName.html', param, function(data) {
                            LoadingMessageBox('close');
                            // 如果存在微课
                            if(data && data.info) {
                                var data = data.info;

                                if (!data.auth[oper_list] || !data.auth[oper_list]['status']) {
                                    return;
                                }
                                // 添加微课播放图标
                                clicked.closest('.test_content').find('.p_knowledge_points .knowledge_point').each(function(w, obj){
                                    var kws = new Array();
                                    var weike = new Array();
                                    weike.version = version;
                                    weike.grade = grade;
                                    weike.unit = unit;
                                    // 知识点id数组
                                    var id_arr = $(this).attr("data-kid").split(',');
                                    $.each(id_arr, function (i, n) {
                                        if (data.cknowledge[n]) {
                                            kws = $.unique($.merge(kws, data.cknowledge[n]));
                                            kws = uniqueArray(kws);
                                            kws.sort();
                                        }
                                    });

                                    $.each(kws, function (j, m) {
                                        if (data.weike[m]) {
                                            weike.push(data.weike[m]);
                                        }
                                    });
                                    // 知识点后微课播放按钮
                                    if(weike.length > 0) {
                                        $(this).after($('#weikePracticeTemp').template(weike));
                                    }
                                });
                            }
                        });
	        ...
	        '.p_answerSubmit_btn:not(.disabled)' : function(){
				...
				// 修改后的练习防护措施
				// 当前练习时间
	    		var duration_pracitce_time = '';
	    		if($('#test-mode[data-mode="free"]').is(':checked')){
		    		duration_pracitce_time = parseInt(time);
		    	}else{
		    		duration_pracitce_time = parseInt(total_time) - parseInt(count_down_time);
				}
				// 答题时间防护
				if(false){
					MessageBox({
						content : '答题时间太短，无法提交本次练习记录。',
						buttons : [{
							text : '继续练习',
							click : function(){
								$(this).dialog('close');
					...
					// 答题数量防护
					var totalNum = $('.p_answer_list ul li').length;
					var practiceNum = $('.p_answer_list ul li.done').length;
					var perVal = practiceNum / totalNum;
					if(false){
						MessageBox({
							content : '大部分题目没有作答，无法提交本次练习记录。',
							buttons : [{
								text : '继续练习',
								click : function(){
									$(this).dialog('close');
					...
					buttons : [
					    {
					    	...
					    	click : function(){
					        	...
					        	if(page == 'record'){
					        		...
					        		// 在线作业
					        		if(source == 'hw'){
					        			var url = 'homework.html?hid=' + id+ '&mode=' + mode;
					        		}
					        		// 试卷练习（单元测试、人机对话和笔试）
					        		else{
					        			if(is_primary){
			...
			'.submit_homework' : function(){
				var endStatus = $('.p_paper_cnt').attr('data-end-status');
				var recScore = $('.H_score').attr('data-total-score');
				if(endStatus == 4){
					if(recScore == 0){
						$content = '本次得分0分，你确定补交作业？';
					}else{
						$content = '确定补交作业？';
					}
				}else{
					if(recScore == 0){ 
						$content = '本次得分0分，你确定提交作业？';
					}else{
						$content = '确定提交作业？';
					}
				}
			...
			//判断选图试题是否做过
			'.test_content[data-type="1100"] .opt_content' : function(){
				var id = $(this).closest('.test_content').attr('data-test-index');
				if($(this).closest('.test_content').find('.images_sort_cnt .dib.imageSize').length > 0 && !$('.p_answer_list ul li[data-index='+id+']').hasClass('done')){
					$('.p_answer_list ul li[data-index='+id+']').addClass('done');
				}
			},
			'.knowledge_point' : function () {
                var count = $(this).attr('data-kcount');
                ...
                if (count >= 1) {
                    ...
                    if(typeof unitId != 'undefined'){
                        ...
                        var unitId = $('.p_paper_cnt').attr('data-unit');
                        if (typeof unitId != 'undefined'){
                            unit = '&unit=' + unitId;
                        }
                    }
                    ...
                    // 打开知识训练
                    var url = 'knowledgePractice.html?knowledge=' + kid + version + grade + unit + '&type=' + url_type;
                    window.open(url);
                }
            },
			// 多个微课的按钮
			'.multi_icon' : function () {
                var html = $(this).closest('.weike_icon_cnt').find('.allweikelist').html();
                $('.multi_weike_list').html(html);
                $('.weike_icon_line_cnt').mCustomScrollbar('destroy').mCustomScrollbar({theme: 'dark'});
                $('.multi_weike_list').dialog({
                    title : '微课列表',
                    width : 500,
                    height : 350,
                    hide : true,
	...
	play_key = setInterval(function(){
		...
		if(pracitce_total_time - remainder_time <= 0){
			...
			if(!flag){
				...
				flag = true;
				// 录音
				ds_tape_video(pracitce_total_time, obj);
			}
		}
	}, 4);
 ...
 * 录音
 */
function ds_tape_video(pracitce_total_time, v_obj){
	// 停止录音
	if(TSP.audio.recorder.inited){
		TSP.audio.recorder.stop();
	}
	...
	$('.test_ctrl_info_area .waveform_container').show();
	// 等待时间
	var remainder_time = Math.ceil(pracitce_total_time) * 1000 * 2;
	// 显示秒
	$('.test_ctrl_info_area .play_mp3_area .remainder_time_area').hide();
 ...
 * 录音
 */
function tape_video(pracitce_total_time, v_obj){
	// 隐藏进度条
	$('.test_ctrl_info_area .percentage_gray').hide();
	// 隐藏录音进度条
	$('.test_ctrl_info_area .waveform_container').show();
...
//	// 答题提示
	...
	play_key = setInterval(function(){
		...
		if(pracitce_total_time - remainder_time <= 0){
			...
			if(!flag){
				// 录音
				tape_video(pracitce_total_time, v_obj);
				// 标识
				flag = true;
			}
		}
